    ...
    useEffect(() => {
        contentRefs.current.forEach((ref, index) => {
            if (ref) {
                ...
                ref.style.height = expandedIndex === index ? `auto` : '50px';
            }
        });
    }, [expandedIndex]);
        ...
        <div className="w-full h-screen snap-section relative">
            <div className="flex flex-col h-full w-full items-center justify-center text-white transition-all transform">
                ...
                <div className="w-[60%] flex flex-col gap-10">
                    {['Who is the CEO of Semi Hyphen?', 'What are the features and options on Semi Hyphen?', 'This year growth in Semi Hyphen?'].map((question, index) => (
                        <div
                            key={index}
